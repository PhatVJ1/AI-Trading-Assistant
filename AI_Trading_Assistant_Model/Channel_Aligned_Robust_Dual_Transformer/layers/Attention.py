    ...
    def _build_from_signature(self, query, value, key=None):
        ...
        with tf_utils.maybe_init_scope(self):
            self._query_dense = core.EinsumDense(
                "btd,thnd->bhnd",
                output_shape= [self._num_heads, self.num_patch, self._key_dim],
                bias_axes="hn",
                name="query",
...
class DualAttentionoverChannels(DualAttentionoverTokens):
    def __init__(self, num_heads: int, key_dim: int, num_patch: int, decay: int, **kwargs):
        ...
        self.DP_K = tf.keras.Sequential([
            tf.keras.layers.Dense(num_patch * num_heads, activation="relu"),
    ...
    def _build_from_signature(self, query, value, key=None):
        ...
        with tf_utils.maybe_init_scope(self):
            self._query_dense = core.EinsumDense(
                "btd,thnd->bhnd",
                output_shape= [self._num_heads, self._query_shape[1], self._key_dim],
                bias_axes="hn",
                name="query",
                **self._get_common_kwargs_for_sublayer(),
            )
