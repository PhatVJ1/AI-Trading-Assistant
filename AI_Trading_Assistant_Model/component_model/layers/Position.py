...
class PositionalTracebackDecoder(tf.keras.layers.Layer):
  ...
  def call(self, x):
    ...
    open = tf.keras.layers.Conv1D(filters=self.d_model, kernel_size=3, padding='same', activation='sigmoid')(open[:, :, tf.newaxis])
    close = tf.keras.layers.Conv1D(filters=self.d_model, kernel_size=3, padding='same', activation='sigmoid')(close[:, :, tf.newaxis])
...
class PositionalTracebackEncoder(tf.keras.layers.Layer):
  ...
  def call(self, x):
    ...
    open = tf.keras.layers.Conv1D(filters=self.d_model, kernel_size=3, padding='same', activation='sigmoid')(open[:, :, tf.newaxis])
    close = tf.keras.layers.Conv1D(filters=self.d_model, kernel_size=3, padding='same', activation='sigmoid')(close[:, :, tf.newaxis])
    volume = tf.keras.layers.Conv1D(filters=self.d_model, kernel_size=3, padding='same', activation='tanh')(volume[:, :, tf.newaxis])
